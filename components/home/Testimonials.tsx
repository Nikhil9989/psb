    ...
    <section ref={ref} className="py-24 bg-white overflow-hidden">
      <div className="container mx-auto px-4">
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="text-xl text-gray-600">Hear from our students who have transformed their careers with our domain-based learning approach.</p>
        </div>
        ...
        >
            ...
            <div className="md:w-1/3 flex flex-col items-center">
              ...
              <div className="flex mt-2">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star key={i} size={18} fill={i < currentTestimonial.rating ? '#F59E0B' : 'none'} stroke={i < currentTestimonial.rating ? '#F59E0B' : '#CBD5E0'} />
                ))}
              </div>
            </div>
          ...
          <div className="flex justify-center mt-8 gap-4">
            <button onClick={prevTestimonial} className="p-2 rounded-full bg-gray-200 hover:bg-gray-300 transition-colors" aria-label="Previous testimonial">
              <ChevronLeft size={20} />
            </button>
            <div className="flex space-x-2">
              {testimonials.map((_, i) => (
                <button key={i} onClick={() => setCurrentIndex(i)} className={`w-3 h-3 rounded-full ${i === currentIndex ? 'bg-blue-600' : 'bg-gray-300'}`} aria-label={`Go to testimonial ${i + 1}`} />
              ))}
            </div>
            <button onClick={nextTestimonial} className="p-2 rounded-full bg-gray-200 hover:bg-gray-300 transition-colors" aria-label="Next testimonial">
